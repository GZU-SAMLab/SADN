...
NUNMGPU=$2
EXPNAME=$3
SAVEDIR=workspace/SADN/coco-det/${EXPNAME}  #<-- change it to your path
...
python3 main.py --num-gpus ${NUNMGPU} --config-file configs/coco/dcfs_det_r101_base.yaml     \
    --opts MODEL.WEIGHTS ${IMAGENET_PRETRAIN}                                         \
           OUTPUT_DIR ${SAVEDIR}/sadn_det_${NET}_base

# ------------------------------ Model Preparation -------------------------------- #
python3 tools/model_surgery.py --dataset coco --method remove                         \
    --src-path ${SAVEDIR}/sadn_det_${NET}_base/model_final.pth                        \
    --save-dir ${SAVEDIR}/sadn_det_${NET}_base

BASE_WEIGHT=${SAVEDIR}/sadn_det_${NET}_base/model_reset_remove.pth

# ------------------------------ Novel Fine-tuning -------------------------------- #
# --> 1. TFA-like, i.e. run seed0~9 (10 times) for FSOD on COCO (20 classes)
classloss="DC" # "CE"
...
do
    ...
    do        
        ...
        OUTPUT_DIR=${SAVEDIR}/sadn_fsod_${NET}_novel/tfa-like-${classloss}/${shot}shot_seed${seed}
        python3 main.py --num-gpus ${NUNMGPU} --config-file ${CONFIG_PATH}                  \
                --opts MODEL.WEIGHTS ${BASE_WEIGHT} OUTPUT_DIR ${OUTPUT_DIR}           \
                       ...
                       TEST.CMCLIP_MODELPATH ${IMAGENET_PRETRAIN_TORCH} \
                       TEST.CMCLIP_MODELTYPE $NET
        #rm ${OUTPUT_DIR}/model_final.pth
    done
done
# surmarize all results
# python3 tools/extract_results.py --res-dir ${SAVEDIR}/sadn_fsod_${NET}_novel/tfa-like-${classloss} --shot-list 1 2 3 5 10 30  

# ----------------------------- Model Preparation --------------------------------- #
python3 tools/model_surgery.py --dataset coco --method randinit                        \
    --src-path ${SAVEDIR}/sadn_det_${NET}_base/model_final.pth                         \
    --save-dir ${SAVEDIR}/sadn_det_${NET}_base
BASE_WEIGHT=${SAVEDIR}/sadn_det_${NET}_base/model_reset_surgery.pth
...
do
    ...
    do
        ...
        OUTPUT_DIR=${SAVEDIR}/sadn_gfsod_${NET}_novel/tfa-like-${classloss}/${shot}shot_seed${seed}
        python3 main.py --num-gpus ${NUNMGPU} --config-file ${CONFIG_PATH}                      \
            --opts MODEL.WEIGHTS ${BASE_WEIGHT} OUTPUT_DIR ${OUTPUT_DIR}               \
                   MODEL.ROI_BOX_HEAD.BBOX_CLS_LOSS_TYPE ${classloss} \
...
# surmarize all results
# python3 tools/extract_results.py --res-dir ${SAVEDIR}/sadn_gfsod_${NET}_novel/tfa-like-${classloss} --shot-list 1 2 3 5 10 30
