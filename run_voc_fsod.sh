...
SAVEDIR=workspace/SADN/voc-det/${EXPNAME}   #<-- change it to your path
...
python3 main.py --num-gpus ${NUNMGPU} --config-file configs/voc/dcfs_det_${NET}_base${SPLIT_ID}.yaml     \
    --opts MODEL.WEIGHTS ${IMAGENET_PRETRAIN}                                                   \
           OUTPUT_DIR ${SAVE_DIR}/sadn_det_${NET}_base${SPLIT_ID}

# ----------------------------- Model Preparation --------------------------------- #
python3 tools/model_surgery.py --dataset voc --method randinit                                \
    --src-path ${SAVEDIR}/sadn_det_${NET}_base${SPLIT_ID}/model_final.pth                    \
    --save-dir ${SAVEDIR}/sadn_det_${NET}_base${SPLIT_ID}
BASE_WEIGHT=${SAVEDIR}/sadn_det_${NET}_base${SPLIT_ID}/model_reset_surgery.pth
...
do
    ...
    do
        ...
        OUTPUT_DIR=${SAVEDIR}/sadn_gfsod_${NET}_novel${SPLIT_ID}/tfa-like-${classloss}/${shot}shot_seed${seed} 
...
# python3 tools/extract_results.py --res-dir ${SAVE_DIR}/sadn_gfsod_${NET}_novel${SPLIT_ID}/tfa-like-${classloss} --shot-list 1 2 3 5 10  
