...
class FastRCNNOutputs(object):
    ...
    def __init__(
        ...
        box_class_loss,
        fs_class,
        pred_class_logits_hn=None,           # === 新增：HN logits ===
        hn_alpha=0.3,                        # === 新增：HN loss 权重 α ===
        hn_tau=0.4,  
    ):
        """
        Args:
        ...
        self.pred_class_logits_hn = pred_class_logits_hn
        self.hn_alpha = hn_alpha
        self.hn_tau = hn_tau
    ...
    def _hard_negative_mask(self):
        """
        在被标注为背景的 ROI 中，挑选那些“容易被误判为前景”的样本。
        判定：前景类最大 softmax 概率 > τ
        返回：Boolean mask，shape [R]
        """
        with torch.no_grad():
            bg_class_ind = self.pred_class_logits.shape[1] - 1
            y = self.gt_classes
            # 前景概率（忽略背景列）
            probs = F.softmax(self.pred_class_logits, dim=1)[:, :-1]  # [R, K]
            max_fg_prob, _ = probs.max(dim=1)                         # [R]
            mask = (y == bg_class_ind) & (max_fg_prob > self.hn_tau)
        return mask
    ...
    def dc_loss_with_hard_negative(self):
        """
        在原 dc_loss 上，叠加 Hard Negative Head 的附加损失：
          loss = dc_loss + α * CE(logits_hn[hn_mask], bg_label)
        """
        base_loss = self.dc_loss()

        if self.pred_class_logits_hn is None:
            return base_loss

        hn_mask = self._hard_negative_mask()
        if hn_mask.sum() == 0:
            return base_loss

        bg_class_ind = self.pred_class_logits.shape[1] - 1
        hn_logits = self.pred_class_logits_hn[hn_mask]     # [Nh, K+1]
        hn_targets = torch.full(
            (hn_logits.size(0),), bg_class_ind,
            dtype=self.gt_classes.dtype, device=self.gt_classes.device
        )
        # 对难负例，强约束其在 HN 分支上预测为背景
        loss_hn = F.cross_entropy(hn_logits, hn_targets, reduction="mean")

        return base_loss + self.hn_alpha * loss_hn
    def dc_loss(self):
        """
    ...
    def losses(self):
        if self.box_class_loss_type == "DC":
            # === 如果有HN分支，走增强损失；否则回落到原dc_loss ===
            cls_loss = (self.dc_loss_with_hard_negative()
                        if self.pred_class_logits_hn is not None
                        else self.dc_loss())
            return {
                "loss_cls": cls_loss,
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        hidden = max(128, input_size // 2)   # 保守选择，避免参数过多
        self.cls_score_hn = nn.Sequential(
    ...
    def forward(self, x):
        ...
        scores_hn = self.cls_score_hn(cls_x)
        return scores, proposal_deltas, scores_hn