...
class ClassGuidedDecoupledAttention(nn.Module):
    def __init__(self, channels, num_classes, reduction=2, spatial_kernels=[3,5,7]):
        super().__init__()
    ...
    def forward(self, x, cls_ids=None):
        ...
        if cls_ids is not None:
            class_vec = self.class_embed(cls_ids)
        else:
            # 如果推理阶段没有类别信息，就用均值
...
class Res5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        self.cgda= ClassGuidedDecoupledAttention(channels=out_channels, num_classes=self.num_classes)
        if self.mask_on:
        ...
        self.cgda_feature = None  # 存储CGDA输出特征图
    ...
    def _shared_roi_transform(self, features, boxes, proposals=None):
    ...
    def forward(self, images, features, proposals, targets=None, fs_class=None):
        ...
        box_features = self._shared_roi_transform(
            [features[f] for f in self.in_features], proposal_boxes, proposals
        )
        feature_pooled = box_features.mean(dim=[2, 3])  # pooled to 1x1
        pred_class_logits, pred_proposal_deltas, pred_class_logits_hn = self.box_predictor(
            feature_pooled
        )
        del feature_pooled
        ...
        outputs = FastRCNNOutputs(
            ...
            self.box_class_loss,
            fs_class,
            pred_class_logits_hn=pred_class_logits_hn 
        )
...
class StandardROIHeads(ROIHeads):
    ...
    def _forward_box(self, features, proposals):
        ...
        pred_class_logits, _, pred_class_logits_hn = self.cls_predictor(
            cls_features
        )

        box_features = self.box_head(box_features)
        _, pred_proposal_deltas, _ = self.box_predictor(
           box_features
        )
        del box_features
        ...
        outputs = FastRCNNOutputs(
            ...
            proposals,
            self.smooth_l1_beta,
            box_class_loss=self.box_class_loss if hasattr(self, "box_class_loss") else "CE",
            fs_class=None,
            pred_class_logits_hn=pred_class_logits_hn  # 新增
        )
        if self.training:
